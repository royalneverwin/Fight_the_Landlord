...
enum class CardComboType
{
    ...
    SINGLE,		// 单张
    PAIR,		// 对子
    STRAIGHT,	// 顺子：5个及以上连续单张
    STRAIGHT2,	// 双顺：3个及以上连续两张
    TRIPLET,	// 三条
    TRIPLET1,	// 三带一
    TRIPLET2,	// 三带二
    BOMB,		// 炸弹
    ...
    QUADRUPLE4, // 四带二（对）
    PLANE,		// 飞机：2个及以上连续三张
    PLANE1,		// 飞机带小翼：每个三张带一个单牌
    PLANE2,		// 飞机带大翼：每个三张带一个对子
    SSHUTTLE,	// 航天飞机：2个及以上连续四张
    SSHUTTLE2,	// 航天飞机带小翼：每个四张带一个单张
    SSHUTTLE4,	// 航天飞机带大翼：每个四张带一个对子
    ROCKET,		// 火箭：王炸
    INVALID		// 非法牌型
};
...
constexpr Level MAX_STRAIGHT_LEVEL = 11; //最大的顺子值
...
{
    return card / 4 + card / 53; //card/53用于区别小王和大王
}
...
struct CardCombo
{
    ...
    */
    ...
    CardCombo(CARD_ITERATOR begin, CARD_ITERATOR end)
    {
        ...
        for (Card c : cards)
            counts[card2level(c)]++;
        for (Level l = 0; l <= MAX_LEVEL; l++) //个人认为这里可以用l < MAX_LEVEL
            if (counts[l])
            {
        ...
        // 计算牌型
        // 按照 同种牌的张数 有几种 进行分类
        vector<int> kindOfCountOfCount; //如果有张数为i的同种牌，push_back
        for (int i = 0; i <= 4; i++)
            if (countOfCount[i])
        ...
        {
            case 1: // 只有一类牌
                curr = countOfCount[kindOfCountOfCount[0]]; //curr代表这一类牌的张数（比如单张的张数）
                switch (kindOfCountOfCount[0])
                {
                    case 1:
                        ...
                        if (curr == 2 && packs[1].level == level_joker) //有两张并且第二张是小鬼（说明第一张是大鬼）
                        {
                            comboType = CardComboType::ROCKET;
                            return;
                        }
                        if (curr >= 5 && findMaxSeq() == curr &&
                            packs.begin()->level <= MAX_STRAIGHT_LEVEL) // 有顺子
                        {
                    ...
                    case 2:
                        ...
                        if (curr >= 3 && findMaxSeq() == curr &&
                            packs.begin()->level <= MAX_STRAIGHT_LEVEL) //有双顺
                        {
                    ...
                    case 3:
                        ...
                        if (findMaxSeq() == curr &&
                            packs.begin()->level <= MAX_STRAIGHT_LEVEL) //有飞机
                        {
                    ...
                    case 4:
                        ...
                        if (findMaxSeq() == curr &&
                            packs.begin()->level <= MAX_STRAIGHT_LEVEL) //有航天飞机
                        {
                ...
                break;
            case 2: // 有两类牌
                curr = countOfCount[kindOfCountOfCount[1]]; //更大的那类
                lesser = countOfCount[kindOfCountOfCount[0]]; //小的那类
                if (kindOfCountOfCount[1] == 3)
                {
                    ...
                    if (kindOfCountOfCount[0] == 1)
                    {
                        ...
                        if (findMaxSeq() == curr && lesser == curr &&
                            packs.begin()->level <= MAX_STRAIGHT_LEVEL) //飞机带单张
                        {
                    ...
                    if (kindOfCountOfCount[0] == 2)
                    {
                        ...
                        if (findMaxSeq() == curr && lesser == curr &&
                            packs.begin()->level <= MAX_STRAIGHT_LEVEL) //飞机带对子
                        {
                ...
                if (kindOfCountOfCount[1] == 4)
                {
                    ...
                    if (kindOfCountOfCount[0] == 1)
                    {
                        ...
                        if (findMaxSeq() == curr && lesser == curr * 2 &&
                            packs.begin()->level <= MAX_STRAIGHT_LEVEL) //航天飞机带单只
                        {
                            comboType = CardComboType::SSHUTTLE2;
                            return;
                        }
                    ...
                    if (kindOfCountOfCount[0] == 2)
                    {
                        ...
                        if (findMaxSeq() == curr && lesser == curr * 2 &&
                            packs.begin()->level <= MAX_STRAIGHT_LEVEL) //航天飞机带对子
                        {
                            comboType = CardComboType::SSHUTTLE4;
                            return;
                        }
...
int main()
{
    ...
    {
        // 做出决策（你只需修改以下部分）
        /*quote: 先把myCards分成不同的Combo*/
        vector<CardCombo> myCardCombs;
        set<Card> myCardsCopy = myCards; //用来分成不同Combo
        sort(myCardsCopy.begin(), myCardsCopy.end());
        /*quote: 先分出来炸弹，再分单顺，再分双顺，再分飞机，最后看其他的*/
        auto i = myCardsCopy.begin();
        while(i != myCardsCopy.end()){ //分出来炸弹
            auto next = i;
            auto nnext = i;
            auto nnnext = i;
            next++;
            if(next != myCardsCopy.end()) {
                nnext = next;
                nnext++;
                if(nnext != myCardsCopy.end()) {
                    nnnext = nnext;
                    nnnext++;
                }
            }
            if(next != myCardsCopy.end() && nnext != myCardsCopy.end() && nnnext != myCardsCopy.end()
            && card2level(*i) == card2level(*next) == card2level(*nnext) == card2level(*nnnext)){ //如果有炸弹
                nnnext++;
                myCardCombs.push_back(CardCombo(i, nnnext));
                auto shouldDelete = i;
                i = nnnext;
                myCardsCopy.erase(shouldDelete, nnnext); //组成Combo之后就从原牌组中去掉
            } else if(next != myCardsCopy.end() && card2level(*i) == level_joker && card2level(*next) == level_JOKER){ //如果有火箭
                myCardCombs.push_back(CardCombo(i, myCardsCopy.end()));
                myCardsCopy.erase(i, myCardsCopy.end()); //组成Combo之后就从原牌组中去掉
                break;
            } else{
                i++;
            }
        ...
        i = myCardsCopy.begin();
        while(i != myCardsCopy.end()){ //分出来单顺（注意不会拆散飞机）

        }
        i = myCardsCopy.begin();
        while(i != myCardsCopy.end()){ //分出来双顺（注意不会拆散飞机）

        }
        i = myCardsCopy.begin();
        while(i != myCardsCopy.end()){ //分出来飞机，不会区分带单张和顺子，使用的时候再看

        }
        i = myCardsCopy.begin();
        while(i != myCardsCopy.end()){ //分出来三条，不会区分带单张和顺子，使用的时候再看
            auto next = i;
            auto nnext = i;
            next++;
            if(next != myCardsCopy.end()){
            ...
            if(next != myCardsCopy.end() && nnext != myCardsCopy.end() &&
                    card2level(*i) == card2level(*next) == card2level(*nnext)){ //如果有三条
                nnext++;

            }
        }
        i = myCardsCopy.begin();
        while(i != myCardsCopy.end()){ //分出来对子
            auto next = i;
            next++;
            if(next != myCardsCopy.end() && card2level(*i) == card2level(*next)){ //是对子
                next++;
                myCardCombs.push_back(CardCombo(i, next));
                auto shouldDelete = i;
                i = next;
                myCardsCopy.erase(shouldDelete, next);
            } else{
                i++;
            }
        }
        i = myCardsCopy.begin();
        while(i != myCardsCopy.end()){ //分出来单张
            auto next = i;
            next++;
            myCardCombs.push_back(CardCombo(i, next));
            i = next;
        ...
        /*quote: 到此为止，所有牌都组成了Combo，根据自己的牌的好坏来叫分*/
        int sumWeight = 0;
        for(auto combo: myCardCombs){
            sumWeight += combo.getWeight();
        }
        int bidValue;
        /*xxx1, xxx2， xxx3的数值待定*/
        if(sumWeight <= xxx1){
            bidValue = 0;
        } else if(sumWeight <= xxx2){
            bidValue = 1;
        } else if(sumWeight <= xxx3){
            bidValue = 2;
        } else{
            bidValue = 3;
        }
